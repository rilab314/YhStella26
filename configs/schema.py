...
class SelfSlotLossConfig(ModuleConfig):
    ...
    # **★ 08-27 되돌림: 0.0.** 위 채택은 U 규격(3,000장)에서만 확인된 것이고, **전체 데이터에서
    # 뒤집혔다** — F03(0.4070)이 대조군 F02(0.4229) 대비 **−3.8%**. F03 은 이 값과 `dir_loss`
    # 둘만 바꾼 판이다. 흔한 종류가 충분히 많아지면 빈도 역가중이 그 종류를 눌러 손해가 된다.
    ...
    # **★ 08-27 되돌림: 0.0.** 전체 데이터에서 **거래였음이 드러났다** — F05(0.4203) vs 대조군
    # F02(0.4229)는 전체 f1 로는 같지만(밴드 안) 길이 구간별로는 **0~20칸 0.206 -> 0.237
    # (+15.0%) · 70칸 이상 0.479 -> 0.444 (−7.3%)** 다. 처방은 의도대로 작동했고 **긴 선을 팔아
    # 짧은 선을 샀다.** 미리 정한 반증 조건("긴 선이 밴드 밖으로 내려가면 기각")에 걸린다.
    # U 규격에서는 긴 선이 −1.3%(밴드 안)였다 — **데이터가 늘면 재분배의 대가가 드러난다.**
    length_power: float = 0.0
...
class ConnLossConfig(ModuleConfig):
    ...
    # **★ 08-27 되돌림: "cosine".** 위 채택도 U 규격 전용이었고 **전체 데이터에서 뒤집혔다** —
    # F03(0.4070)이 F02(0.4229) 대비 −3.8%. `class_freq_power` 와 한 판에 같이 들어가 있어
    # 둘 중 어느 쪽이 손해인지는 못 가른다. 둘 다 되돌린다.
    dir_loss: str = "cosine"
...
class DecodeConfig(ModuleConfig):
    ...
    # 정점의 전경 판정을 **배경 확률 문턱**으로 바꾼다 (0이면 무동작 = 기존 argmax 규칙).
    # 근거(08-22 실측, val 120장): 정답 칸이 히트맵 문턱까지는 0.889 통과하는데 전경 관문을
    # 지나면 0.291 만 남는다 — 70칸 이상 선(0.967 -> 0.500)보다 짧은 선이 유독 더 깎인다.
    # `argmax != 0` 은 가장 빡빡한 규칙이라 애매한 칸이 전부 배경이 된다.
    # **08-22 무효 — 기본값 0(끄기)을 유지한다.** 문턱을 풀어도 f1 0.3860 -> 0.3890(+0.8%,
    # 밴드 안)에 그치고 0.7 에서는 0.3416(-11.5%)로 무너진다. 재현율은 1.5% 밖에 안 오른다 —
    # **모델이 그 칸들을 자신 있게 배경이라 부르고 있다.** 처방은 디코더가 아니라 학습 쪽이다.
    # 손잡이는 논문의 디코더 민감도 ablation 용으로 남긴다.
    bg_prob_max: float = 0.0
    fg_thresh: float = -1.0
