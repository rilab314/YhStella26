...
Y·T자 분기는 "끝칸 미채움" 규약이, X·+자 교차는 셀 소유권 + 디코더 탐색 반경이 처리한다(6.4절).
...
4. **셀 소유권 (선 사이).** 여러 선이 같은 셀에 픽셀을 남기면 **픽셀이 더 많은 선이 소유한다**
   ...
   그 칸을 건너뛰고, 디코더의 탐색 반경(`decode.radius`, 기본 5셀)이 이를 복원한다(10.3절).
...
7. **`conn_dirs`.** 사슬의 앞·뒤 이웃에 대해 **자기 점 → 이웃 점 단위 방향을 직접 계산해
   ...
   - **이웃 칸 접선은 스텝마다 흔들린다** — 인코더로 직접 잰 방향 요동이 평균 5.9°, 90%분위
     14.0°, 20°를 넘는 스텝이 4.4%다(val 40장·사슬 1,475개). 모델의 방향 오차가 이와 비슷한
     수준이라 **목표 자체의 잡음이 하한을 만들고 있을 수 있다.** 몇 칸 앞을 보게 하는 손잡이
     (`data.conn_lookahead`, 기본 1)가 열려 있지만 **1 이외의 값은 쓰지 않는다** — 완벽한 예측을
     넣어도 GT 주입 천장이 k=1 0.977 → k=2 0.849로 무너진다(기전 미해명). 값을 바꾸면 GT 캐시가
     낡으므로 캐시 폴더 이름에 이 값이 들어간다.
...
실데이터는 **SEED-MAP v1.2**(Shin & Choi, IEEE JSTARS vol. 18, 2025 의 v1.1 을 정리한 판)이다.
위성 영상 768×768, 지상 해상도 **0.2550 m/px**, 타일당 지상 범위 **195.8 m**.

**옛 문서값 0.1278 m/px 은 원해상도(1536 px) 타일 기준이다** — 우리가 쓰는 768 px 타일은
그 절반 크기이므로 값이 두 배가 된다. 전 타일의 MetaData footprint 가
0.00221429° × 0.00176495° 로 일정하고, 37.5°N 에서 195.80 m × 195.89 m / 768 px = **0.2550**.
교차 검증: 차선 간격 중앙값 11.8 px × 0.255 = **3.01 m** 로 실제 차로 폭과 맞는다
(0.1278 이면 1.51 m 라 물리적으로 불가능하다). **미터로 적히는 값은 전부 이 값으로 읽는다.**
...
원본 `SEED_MAP_v1.2`는 평평한 `image/`·`label/` + `dataset.json`(`{train, validation, test}` 키)으로
split을 표현한다. 로더는 그 원본이 아니라 **split 폴더 구조로 재정리한 사본**을 읽는다 —
`SEED_MAP_v1.2_splits/`에 **복사**(사용자 결정 — 링크·이동 아님. 원본 무손상, 완전 독립 사본).
재정리는 `scripts/build_split_dataset.py`가 한다. **라벨 JSON은 변환 없이 그대로 복사한다** —
원본 형식을 로더가 직접 읽으므로 재정리는 파일 배치만 바꾼다.

```
{data.root} = .../2026_stella/SEED_MAP_v1.2_splits/
├── train/  image/{id}.png   label/{id}.json      # 8,979장
├── val/    image/{id}.png   label/{id}.json      # 1,218장 — 원본 "validation" → "val"
└── test/   image/{id}.png   label/{id}.json      # 2,457장
```

**v1.1 에서 무엇이 바뀌었나.** 두 가지이고 둘 다 val·test 쪽 정리다.

1. **split 사이로 화면이 새던 타일 174장을 뺐다.** 타일은 196 m 정사각형인데 80~100 m 격자
   위에 놓여 있어 이웃끼리 절반 넘게 겹친다. 학습 타일과 겹치는 검증 타일, 그리고 살아남은
   검증 타일과 겹치는 시험 타일을 뺐다 (val −64 · test −110). **학습 타일은 하나도 빼지 않는다.**
2. **타일 밖으로 완전히 나간 폴리라인을 원본에서 지웠다.** 이 항목은 **우리 로더에 영향이 없다** —
   6.7.4 규칙 4(이미지 경계로 자르기)가 그런 선을 이미 조각 0개로 버리고 있었다.
   라벨이 편집된 train 타일 1,205장을 v1.1·v1.2 양쪽으로 읽어 **로더 출력이 완전히 같음을 확인**했다.

**따라서 GT 캐시를 다시 뜨지 않는다.** 캐시는 타일 이름별 `.npz` 라 빠진 타일의 캐시가
쓰이지 않을 뿐이고, 남은 타일의 인코딩 결과는 바이트 단위로 같다.

**과거 실행의 성적은 다시 재지 않는다** (사용자 판단 — 빠진 174장이 성능을 크게 흔들지 않는다).
다만 **v1.1 val 1,282장에서 잰 값과 v1.2 val 1,218장에서 잰 값은 서로 다른 표본**이므로,
논문 표처럼 한 줄에 나란히 놓을 값은 같은 판에서 다시 잰다.
`gate_baseline.json`의 하한은 **고정된 예측 캐시**에서 재므로 데이터셋 교체의 영향을 받지 않는다.
...
| 건너뛴 간선(체비셰프 ≥ 2) | 0.76% | 소유권 손실 칸을 사슬이 건너뛴 비율 — **GT 간선의 통계**다(98.7%가 1칸·1.3%가 2칸). 여기서 `radius = 2`가 나왔으나 **그 값은 정점 재현율이 100%라는 전제 위에 선다** — 실제 모델은 GT 셀의 ~44%만 정점으로 만들어서 이웃한 두 GT 셀이 둘 다 예측되는 일이 드물다. 디코더는 그 빈칸을 건너뛰어야 하므로 더 큰 반경이 필요하다. 다만 **반경의 상한을 정하는 것은 이 통계가 아니라 차선 간격**이고, 그래서 현재 값은 5셀이다 (10.3절) |
| 끝 셀 비율 | **4.12%** (of nodes) | 끝칸 미채움 후 양성 셀 중 끝 셀 비중 — 8.2절 `end_pos_weight` 불균형 논의의 분모 |

**차선 간 거리(수직 최근접) 중앙값은 11.8 px다.** 이 하나의 값이 세 파라미터를 정한다 —
평가 버퍼 $\rho = 3$ px(11.1절), 디코더 탐색 반경 5셀 = 20 px(10.3절), 중복 정리 문턱 3 px
(10.4.1절). 셋 다 **"이웃 차선을 자기 것으로 착각하지 않을 만큼 좁게"** 라는 같은 규칙에서
나왔다. 미터로는 11.8 px × 0.255 = **3.01 m**로 실제 차로 폭과 맞는다.
