...
형식으로 주입하면 인스턴스 F1 ≈ 1**이고, 실측 **0.976**으로 통과했다
(ConvNeXtV2-base 대역 config 기준).
...
1. **후보 집합.** $a$의 셀에서 체비셰프 반경 `radius`(= 24) 안의 미사용 정점 $b$ 중
...
$$
   ...
   여기서 $w_{dist}$는 셀 단위 거리에 곱하는 계수다(`decode.w_dist`, 기본 **0.03**).

   **이 값은 탐색 반경과 함께 정해진다.** 반경 2 시절에는 거리 항이 해로웠고
   (`w_dist = 0.3` → 간선 정확도 0.985 → 0.953) 동률 해소용 0.001만 남겼는데, 반경을 24로
   키운 뒤 그 값을 다시 재지 않았다. 반경 24에서 0.001의 최대 기여는 0.034로 정렬 항
   $(1-\text{align})$의 범위 $[0, 0.3]$ 앞에서 무시되고, 그 결과 **디코더가 정렬만 조금
   나으면 20셀을 건너뛰며 정점을 지나쳐** 한 선이 4~5조각으로 쪼개졌다. 0.03으로 올리면
   f1 +33% · 정밀도 +64% · `chain_len` +130% · `frag` −38%다(캐시 4개 · 200장 일정).
   0.03~0.04가 평평한 최적이다.
...
0. **조각 병합 (`ChainMerger`, 개선 루프 가설 백로그).** 사슬 확장이 다 끝난 폴리라인 목록에서,
   ...
   기본값이 꺼짐인 이유는 사슬 확장 자체가 `radius`로 소유권 손실 칸을 건너뛰기 때문이다
...
**버퍼 폭 $\rho$ — 2026-08 재설정.** 원래 계획은 "차선 간격의 절반 이하"였는데 기본값
...
- 예측이 **중간에 이웃 차선으로 갈아타도** 매 순간 어떤 GT 버퍼 안에는 있어서 만점을 받았다.
  실측(08-14): 매칭 자격을 얻던 예측의 **17%가 갈아탄 것**이었다 (GT 주입에서는 1%).
