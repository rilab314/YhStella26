...
| 37   | 논문 그림·표 생성 방식     | **산출물 하나 = 스크립트 하나** (사용자 지시 08-26). `scripts/paper/`에 그림 14개·표 9개를 각각 별도 스크립트로 둔다. 정성 그림은 정량 조건으로 후보를 고르고 **상한 100장**까지만 낸다. 출력은 저장소가 아니라 데이터 작업 폴더에 **폴더 단위로 지우고 다시 쓴다** (15절) |
